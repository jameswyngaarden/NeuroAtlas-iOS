// CoordinateTransformer.swift - Improved precision with reduced snapping
import Foundation
import SwiftUI
...
struct CoordinateTransformer {
    static func screenToMNI(screenPoint: CGPoint, containerSize: CGSize, slice: BrainSlice) -> MNICoordinate {
        ...
        // Convert to slice pixel coordinates with higher precision
        ...
        // Convert pixel coordinates to MNI coordinates using bounds with better precision
        ...
        case .coronal:
            let rawMniX = Double(bounds.xMin) + (pixelX / sliceWidth) * Double(bounds.xMax - bounds.xMin)
            mniY = slice.mniPosition
            let rawMniZ = Double(bounds.zMin) + (pixelY / sliceHeight) * Double(bounds.zMax - bounds.zMin)
            
            mniX = Int(round(rawMniX))
            mniZ = Int(round(rawMniZ))
            
            print("Coronal calculation:")
            print("  mniX = \(bounds.xMin) + (\(pixelX)/\(sliceWidth)) * \(bounds.xMax - bounds.xMin) = \(rawMniX) → \(mniX)")
            print("  mniY = \(slice.mniPosition) (fixed)")
            print("  mniZ = \(bounds.zMin) + (\(pixelY)/\(sliceHeight)) * \(bounds.zMax - bounds.zMin) = \(rawMniZ) → \(mniZ)")
            
        case .axial:
            let rawMniX = Double(bounds.xMin) + (pixelX / sliceWidth) * Double(bounds.xMax - bounds.xMin)
            let rawMniY = Double(bounds.yMin) + (pixelY / sliceHeight) * Double(bounds.yMax - bounds.yMin)
            ...
            print("  mniX = \(bounds.xMin) + (\(pixelX)/\(sliceWidth)) * \(bounds.xMax - bounds.xMin) = \(rawMniX) → \(mniX)")
            print("  mniY = \(bounds.yMin) + (\(pixelY)/\(sliceHeight)) * \(bounds.yMax - bounds.yMin) = \(rawMniY) → \(mniY)")
